...
export class AuthService {
  ...
  async registerWithGoogle(supabaseAccessToken: string): Promise<AuthResponse> {
    const response = await this.api.post<ApiResponse<AuthResponse>>(API_ROUTES.v1.auth.google, {
  ...
  async proactivelyRefreshAccessToken(): Promise<boolean> {
    try {
      ...
      if (refreshResponse.data?.data?.access_token) {
        tokenService.setAccessToken(refreshResponse.data.data.access_token);
        return true;
      } else {
        console.warn(
