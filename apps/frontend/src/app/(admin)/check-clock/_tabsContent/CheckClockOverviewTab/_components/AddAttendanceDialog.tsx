...
export const AddAttendanceDialog = ({
	...
	const filteredEmployeeList = React.useMemo(
		() =>
			employeeList.filter(
				(employee) =>
					employee.work_schedule_id && employee.work_schedule
			),
		[employeeList]
	);
			...
			if (isClockInOut) {
				...
				switch (data.attendanceType) {
					...
					case "compassionate leave":
						leaveType = LEAVE_TYPE.COMPASSIONATE_LEAVE;
						break;
					case "maternity leave":
						leaveType = LEAVE_TYPE.MATERNITY_LEAVE;
						break;
					case "annual leave":
						leaveType = LEAVE_TYPE.ANNUAL_LEAVE;
						break;
					case "marriage leave":
						leaveType = LEAVE_TYPE.MARRIAGE_LEAVE;
						break;
					default:
				...
				await createLeaveRequestMutation.mutateAsync({
					employeeId: parseInt(data.employee_id, 10),
					data: leaveData,
				});
									...
									>
										<PopoverTrigger asChild>
											...
											>
												{formData.employee_id
													? (() => {
															const foundEmployee = filteredEmployeeList.find(
																(employee) =>
																	employee.id.toString() ===
																	formData.employee_id
															);
															return foundEmployee
																? `${foundEmployee.first_name} ${foundEmployee.last_name}`
																: "Select employee...";
													  })()
													: "Select employee..."}
