...
import { ApiResponse } from "@/types/subscription.types"; // Assuming you have a common ApiResponse type
import { formatAttendanceStatus } from "@/utils/status";
...
class AttendanceService {
	...
	async getEmployeeMonthlyStatistics(
		...
		const url = API_ROUTES.v1.api.attendances.monthlyStatistics(
			year,
			month
		);
		...
		try {
			const response = await this.apiService.get<
				ApiResponse<EmployeeMonthlyStatistics>
			>(url);
			return response.data.data;
		} catch (error) {
			console.error(
				"AttendanceService: Error fetching monthly statistics:",
				error
			);
			throw error;
		}
