...
export function PageSizeComponent<TData>({
  ...
  const actualTotalRecords = totalRecords ?? table.getFilteredRowModel().rows.length;
  ...
  const firstRecordOnPage = currentPageRows > 0 ? pageIndex * pageSize + 1 : 0;
  const lastRecordOnPage = firstRecordOnPage > 0 ? firstRecordOnPage + currentPageRows - 1 : 0;

  return (
    <div className='flex w-full items-center gap-3 sm:w-auto'>
      <span className='hidden text-sm text-slate-600 sm:inline dark:text-slate-400'>Rows</span>
