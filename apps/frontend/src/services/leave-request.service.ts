...
class LeaveRequestService extends ApiService {
  ...
  async createLeaveRequest(data: CreateLeaveRequestRequest): Promise<LeaveRequest> {
    ...
    if (data.employee_note) formData.append('employee_note', data.employee_note);
  ...
  async updateLeaveRequest(id: number, data: UpdateLeaveRequestRequest): Promise<LeaveRequest> {
    const formData = new FormData();
