...
export function ClockInOutDialog({
	...
	// Add time validation for check-in window
	const isWithinCheckInTime = React.useMemo(() => {
		if (isWFA || !todaySchedule || actionType !== "clock-in") return true;
		...
		const startDateTime = new Date(`${today}T${checkinStartUTC}Z`);
		const endDateTime = new Date(`${today}T${checkinEndUTC}Z`);
	...
	}, [isWFA, todaySchedule, actionType]);
	...
	const getTimeValidationMessage = () => {
		...
		if (!checkinStartUTC || !checkinEndUTC) return null;
		...
		if (!isWithinCheckInTime) {
			return `Clock-in is only allowed between ${checkinStartLocal} and ${checkinEndLocal} (local time)`;
		}

		return null;
	};

	// Add early clock-out detection
	const isEarlyClockOut = React.useMemo(() => {
		if (isWFA || !todaySchedule || actionType !== "clock-out") return false;

		const checkoutStartUTC = todaySchedule.checkout_start;
		if (!checkoutStartUTC) return false;
		...
		const today = new Date().toISOString().split("T")[0];
		const checkoutStartDateTime = new Date(`${today}T${checkoutStartUTC}Z`);
	...
	const validateLocation = React.useCallback(
		(userLat: number, userLon: number) => {
			...
			setLocationValidation({
				...
				message: isValid
					? `You are within the allowed work location (${
							workLocation.name
					  }). Distance: ${Math.round(distance)}m`
					: `You are ${Math.round(
							distance
					  )}m away from the allowed work location (${
							workLocation.name
					  }). Maximum allowed distance is ${workLocation.radius}m.`,
				hasRefreshed: true,
			});
	...
	const handleFormSubmit = (data: AttendanceFormData) => {
		...
		if (
			...
			const distanceKm =
				Math.round(((locationValidation.distance || 0) / 1000) * 100) /
				100;
			toast.error(
													...
													>
														{(() => {
															...
															return isWithinCheckInTime
																? `You can clock in now. Allowed time: ${checkinStartLocal} - ${checkinEndLocal} (local time)`
																: `Clock-in is only allowed between ${checkinStartLocal} and ${checkinEndLocal} (local time)`;
														})()}
													</p>
													{!isWithinCheckInTime && (
