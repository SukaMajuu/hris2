...
export function useUserProfile(): UserProfile {
  ...
  const { data: currentEmployee, isLoading, error } = useCurrentUserProfileQuery();
  ...
  const userProfile = useMemo(() => {
    ...
    const initials = currentEmployee
      ? `${currentEmployee.first_name?.charAt(0) || ''}${currentEmployee.last_name?.charAt(0) || ''}`.toUpperCase() ||
        'U'
