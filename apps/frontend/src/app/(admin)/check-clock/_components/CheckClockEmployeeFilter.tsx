...
export function CheckClockEmployeeFilter({
	onApplyFilters,
	onResetFilters,
	currentFilters,
	isVisible,
	...
	return (
		<Card className="border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-sm mb-6">			<CardContent className="pt-6">
				<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
					...
					{/* Position Filter */}
					<div className="space-y-2 min-w-0">
						<Label htmlFor="position-filter" className="text-sm font-medium text-slate-700 dark:text-slate-300 flex items-center gap-2">
					...
					{/* Work Type Filter */}
					<div className="space-y-2 min-w-0">
						<Label htmlFor="work-type-filter" className="text-sm font-medium text-slate-700 dark:text-slate-300 flex items-center gap-2">
							<Briefcase className="h-4 w-4 flex-shrink-0" />
							<span className="truncate">Work Type</span>
						</Label>
						<Select
							value={localFilters.work_type || "all"}
							onValueChange={(value) => handleInputChange("work_type", value === "all" ? "" : value)}
						>
							...
							<SelectContent>
								...
								{Object.values(WORK_TYPES).map((workType) => (
									<SelectItem key={workType} value={workType} className="flex items-center gap-2">
										<div className="flex items-center gap-2">
											{getWorkTypeIcon(workType)}
											<span>
												{workType === "WFO" && "Work From Office (WFO)"}
												{workType === "WFA" && "Work From Anywhere (WFA)"}
												{workType === "WFH" && "Work From Home (WFH)"}
												{workType === "Hybrid" && "Hybrid Work"}
											</span>
										</div>
									</SelectItem>
								))}
					...
					<div className="space-y-2 min-w-0">
						<Label htmlFor="schedule-filter" className="text-sm font-medium text-slate-700 dark:text-slate-300 flex items-center gap-2">
							<Clock className="h-4 w-4 flex-shrink-0" />
							<span className="truncate">Work Schedule</span>
						</Label>
						<Select
							value={localFilters.work_schedule_id || "all"}
							onValueChange={(value) => handleInputChange("work_schedule_id", value === "all" ? "" : value)}
						>
							<SelectTrigger className="w-full bg-white dark:bg-slate-800 border-slate-300 dark:border-slate-600 hover:border-slate-400 dark:hover:border-slate-500 transition-colors">
							...
							<SelectContent>
								...
								{workSchedules.map((schedule) => (
									<SelectItem 
										key={schedule.id} 
										value={schedule.id?.toString() || `schedule-${schedule.name}`}
