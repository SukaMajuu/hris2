...
export function UserDashboardCharts({
	...
	const getDateRange = (period: string): string => {
		...
		if (period === "This Week") {
			...
			return `(${formatDate(mondayLastWeek)}–${formatDate(
				saturdayLastWeek
			)})`;
	...
	useEffect(() => {
		if (isDropdownOpen && optionsContainerRef.current) {
			const selectedIndex = monthYearOptions.findIndex(
	...
	return (
		<div className="grid grid-cols-1 gap-6 xl:grid-cols-2">
			{/* Attendance Summary */}
			<Card className="rounded-2xl border border-gray-100 p-0 shadow-lg hover:shadow-xl transition-shadow duration-300 bg-white">
							...
							{canAccessCheckClock && (
								<div className="relative" ref={dropdownRef}>
									...
									>
										...
										<ChevronDown
											className={`h-4 w-4 ml-3 transition-transform duration-200 text-gray-500 ${
												isDropdownOpen
													? "rotate-180"
													: ""
											}`}
									...
									{isDropdownOpen && (
										<div
											className="absolute top-full left-0 right-0 mt-2 bg-white border border-gray-200 rounded-lg shadow-xl z-50 max-h-64 overflow-y-auto backdrop-blur-sm"
											ref={optionsContainerRef}
										>
											<div className="py-2">
												{monthYearOptions.map(
													(option) => (
														<button
															key={option.value}
															type="button"
