...
const getStatusBadge = (status: string) => {
  switch (status.toLowerCase()) {
    ...
    case 'waiting_approval':
      return <Badge className="bg-yellow-100 text-yellow-800 border-yellow-200">Waiting Approval</Badge>;
    default:
...
export default function PermitTab() {
  ...
  const { data: leaveRequestsData, isLoading, error, refetch } = useMyLeaveRequests(currentPage, pageSize);
  
  const [openSheet, setOpenSheet] = useState(false);
  const [selectedDetail, setSelectedDetail] = useState<PermitDetail | null>(null);
  ...
  const handleViewDetails = useCallback((id: number) => {
    const request = permitData.find((item: PermitData) => item.id === id);
    if (request) {
      setSelectedDetail({
        id: request.id,
  ...
  const onSubmit = async (data: DialogFormData) => {
    try {
      ...
      toast({
        ...
        description: 'Terjadi kesalahan saat mengajukan permohonan. Silakan coba lagi.',
        variant: 'destructive',
      });
    }
  };
  ...
  return (
    <>
            ...
            <div className="mx-2 space-y-6 text-sm sm:mx-4">
              <div className="mb-6 rounded-lg bg-white p-6 shadow-md">
                ...
                <p className="text-sm text-slate-500">
                  Submitted on {new Date(selectedDetail.submittedAt).toLocaleDateString()}
                </p>
                <div className="mt-2">
              ...
              <div className="rounded-lg bg-white p-6 shadow-md">
                ...
                <div className="grid grid-cols-1 gap-x-6 gap-y-4 md:grid-cols-2">
                  <div>
                    ...
                    <p className="text-slate-700">{new Date(selectedDetail.startDate).toLocaleDateString()}</p>
                  </div>
                  <div>
                    <p className="text-xs font-medium text-slate-500">End Date</p>
                    <p className="text-slate-700">{new Date(selectedDetail.endDate).toLocaleDateString()}</p>
                  </div>
                  <div className="col-span-1 md:col-span-2">
                    <p className="text-xs font-medium text-slate-500">Reason</p>
                    <p className="text-slate-700">{selectedDetail.reason}</p>
                  </div>
                  {selectedDetail.adminNote && (
                    <div className="col-span-1 md:col-span-2">
                      <p className="text-xs font-medium text-slate-500">Admin Note</p>
                      <p className="text-slate-700">{selectedDetail.adminNote}</p>
                    </div>
                  )}
                </div>
              </div>
      ...
      </Sheet>      {/* New Leave Request Dialog */}
      <PermitDialog
        open={openDialog}
