...
const CheckClockApprovalTab = () => {
	...
	const handleApproveWithNote = useCallback(
		async (noteText: string) => {
			try {
				await handleApprove(noteText);
			} catch (approveError) {
				console.error("Error approving with note:", approveError);
			}
		},
		[handleApprove]
	);
	...
	const handleRejectWithNote = useCallback(
		async (noteText: string) => {
			try {
				await handleReject(noteText);
			} catch (rejectError) {
				console.error("Error rejecting with note:", rejectError);
			}
		},
		[handleReject]
	);
...
export default CheckClockApprovalTab;
