...
function validateRow(row: Record<string, string>, rowIndex: number): ImportValidationError[] {
  ...
  // Validate dates
  if (row.date_of_birth && !validateDate(row.date_of_birth.toString().trim())) {
    errors.push({
      row: rowIndex,
      field: 'date_of_birth',
  ...
  // Validate age
  if (row.date_of_birth && !validateAge(row.date_of_birth.toString().trim())) {
    errors.push({
      row: rowIndex,
      field: 'date_of_birth',
...
export function parseEmployeeCSV(file: File): Promise<ImportResult> {
        ...
        dataRows.forEach((row, index) => {
          ...
          if (rowErrors.length === 0) {
            ...
            const cleanRow: EmployeeImportData = {
              ...
              date_of_birth: rowData.date_of_birth?.toString().trim() || undefined,
              ...
              contract_type:
                (rowData.contract_type?.toString().trim() as
                  ...
                  | 'contract'
                  | 'freelance') || undefined,
              hire_date: rowData.hire_date?.toString().trim() || undefined,
